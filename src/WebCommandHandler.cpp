...
#define ASSIGN_C_STRING(addr, dataLength, str)      \
    strncpy((char *)addr, str.c_str(), dataLength); \
    ((char *)addr)[dataLength] = 0;
...
#define IF_CAST(type, _, ASSIGN)                            \
    if (config.typeName == F(#type)) {                      \
        ASSIGN(settings[key], ((type *)config.dataPointer)) \
    }
...
#define IF_ASSIGN(type, ASSIGN, _)                                     \
    if (config.typeName == F(#type)) {                                 \
        ASSIGN(((type *)config.dataPointer), config.dataLength, value) \
    }
...
void display(AsyncWebSocketClient *client, std::vector<String> &args) {
    if (args.size() == 0) {
        ...
        if (args[0] == F("colorcorrection")) {
            ...
            if (args.size() == 1) {
                RESPOND(client, F("Subcommands: cycle, radar, rainbow, rainbowWithGlitter, noise, waves, confetti, sinelon, juggle, flashing, bpm, solid"));
            } else {
                args[1].toLowerCase();
                if (args[1] == "cycle") {
...
void onEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {
    if (type == WS_EVT_CONNECT) {
        ...
        if (info->final && info->index == 0 && info->len == len) {
            ...
            data[info->len] = 0;
            handleMessage(client, String((char *)data).c_str());
            DEBUG_DUMP_CONFIG(client);
        } else if (info->final) {
